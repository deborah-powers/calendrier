...
$mysqli = new mysqli ($DBhost, $DBuser, $DBpassword, $DBname) or die ($mysqli->error);

function findObjectInDb ($tableName, $obj, $mysqli){
	...
	$response = $mysqli->query ($request) or die ($mysqli->error);
	$tableData = mysqli_fetch_all ($response);
	return $tableData;
}
function isObjInDB ($tableName, $obj, $mysqli){
...
function updateObjToDb ($tableName, $obj, $mysqli){
...
function getTableContent ($tableName, $mysqli){
...
function tableToJsonArray ($tableFields, $tableData){
